...
// .reduce() - виконує додавання елементів масивів самостійно. Застосовує фунцкію reducer до кожного елемента масива (з ліва на право), повертаючи одне результуюче значення
...
// Метод reduce() застосовує функцію-колбек до кожного елемента масиву (зліва направо) і зводить його до одного значення. Для підрахунку суми ми можемо використовувати його наступним чином:

// const numbers = [10, 5, 8, 20, 3];
// const sum = numbers.reduce((accumulator, currentValue) => accumulator + currentValue, 0);

// console.log(sum); // Виведе: 46

// Де:
// numbers: Ваш масив чисел.
// reduce(): Метод масиву, який використовується для зведення масиву до одного значення.
// (accumulator, currentValue) => accumulator + currentValue: Функція-колбек, яка виконується для кожного елемента масиву.
// accumulator: Акумулятор, який зберігає проміжне значення суми. На першій ітерації він приймає значення initialValue (у нашому випадку 0).
// currentValue: Поточний елемент масиву, який обробляється.
// => accumulator + currentValue: На кожній ітерації поточне значення додається до акумулятора.
// 0: Початкове значення (initialValue) для акумулятора. Якщо не вказати, то акумулятор буде ініціалізований першим елементом масиву, а ітерація почнеться з другого елемента.
...
// ... - оператор три крапки називається Spread оператор, він використовується для різних цілей
// Наприклад багато вбудованих функцій JS підтримують довільну кількість аргументів. Наприклад Math.max(arg1, arg2, ..., argN)
//Також його викориостувають при копіюванні масивів обєктів тощо для того щоб розпакувати вміст масива який копіюється в масив в який копіюємо
...
// До тепер ми вчились писати функції з конкретною кількістю параметрів що в неї передаються, і знаємо що якщо параметрів буде більше ніж аргуменів чи навпаки будуть виникати помилки типу undefined
// Щоб написати функцію яка може приймати невизначену кількість параметрів (наприклад коли ми не можемо знати які саме параметри будуть передаватись в функцію, або якщо їх кількість може коливатись в залежності від задачі...), 
// треба використати підхід arguments

// argumnets - це абстрактий тип даних. Спосіб щось зберігати разом. Це властивість по замовчуванню будь якої функції
// arguments - це такий вид колекції, набір елементів, для збереження аргументів функції
// argumnets - це псевдомасив, він має всі ті ж методи та властивоссті що й масив, звертаємось до елементів так само як в масиві, але не забуваєм що це НЕ масив, тому до нього НЕ можна використовувати методи масивів. Можна лише пошук за індексом, length, та цикли for i for...of 

// використовується arguments в тілі фунцкії

// function f() {  // не оголошуємо жодних параметрів
//         console.log(arguments[0]);  // виведе 1 при тому що параметри в нас відсутні
//         console.log(arguments[1]);  // виведе 2 при тому що параметри в нас відсутні
//         console.log(arguments);
// }
// f(1, 2);  // передаємо два аргументи
// console.dir(f);  // перевіряємо що в функції arguments є вбудованою властивістю


// function f(a, b) {  // або можем оголисити параметрів менше ніж будес передавати аргументів
//         console.log(arguments[0]);  // виведе 1, 1 аргумент попаде в 1 параметр
//         console.log(arguments[1]);  // виведе 2, 2 аргумент попаде в 2 параметр
//         console.log(arguments);   // Arguments(4) [1, 2, 3, 4, callee: ƒ, Symbol(Symbol.iterator): ƒ] - arguments всерівно буде включати в себе всі елементи навіть ті що вже потрапили в параметри.
// }
// f(1, 2, 3, 4);  // передаємо 4 аргументи, де 1 аргумент потрапить в перший параметр функції, 2 аргумент потрапть в другий параметр функції, але в arguments всерівно потраплять всі передані аргументи, навіть ті, що вже потрапили в параметри функції
// console.dir(f); 


// Написати функцію щоб порахувати суму необмеженої кількості елементів

// function sum() {
//         // console.log(arguments);  // Arguments(7) [1, 2, 3, 4, 5, 6, 7, callee: ƒ, Symbol(Symbol.iterator): ƒ] - перевіряємо чи всі аргументи пройшли в arguments

//         let sum = 0; 
//         for (let i = 0; i < arguments.length; i++) {
//                 sum += arguments[i]; // сумуємо значення елементів 
//         }
//         return sum;
// }

// const sumOfArguments = sum(1, 2, 3, 4, 5, 6 ,7); // передаємо будь яку кількість аргументів, при цьому ми зовсім не зазначили параметри
// console.log(sumOfArguments);


// Задачка
//Написати функцію для розрахунку добутку необмеженої кількості переданих аргументів
...
//         return sum;
// }
// const multOfArguments = multiplication(2, 4, 6);
// console.log(multOfArguments);


// Задачка
// Написати функцію виду
// calculate(operation, operand1, operand2, ..., operandN)
// де operation- це операція, яка має бути виконана над операндами.arguments (+, *)
// operation - arguments[0], тому цикл перебираємо з 1


                // operation = arguments[0], operand1 = arguments[1]...
// function calculate(operation, operand1, operand2, ... operandN) {
//         let accum;
//         if (operation === '+') {
//                 accum = 0;
...
// const sumCalculate = calculate('*', 2, 4, 6);
...
// rest parameters - дуже схожа штука на arguments, але має певні суттєві відмінності
//1. пишеться в параметри функції через знак ...+назва довільна параметру (можна rest можна args чи будь як інакше)
//2. його потрібно оголосити в параметрах обовязково
//3. якщо ми додатково оголосимо й інші параметри типу f(a, b, ...args) - то 1 аргумент піде в перший парам, 2 аргумент піде в 2 парам, а всі інші аргументи підуть в rest-параметри (тобто в рест підуть лише ті, які не потраплять в інші оголошені параметри)
//4. якщо нам потрібно оголосити ще якісь параметри крім rest : function f(a, b, ...args), то rest завжди повинен бути останнім!
//5. Тип даних Array



// function f(...args) {
//         console.log(args); // в rest параметр потраплять усі аргументи які ми передаємо і запаковує його в масив, тобто доступні всі методи масивів для подальшої обробки
// }

// const res = f(1, 2, 3, 4, 5);

//якщо ми додатково оголосимо й інші параметри типу f(a, b, ...args) - то 1 аргумент піде в перший парам, 2 аргумент піде в 2 парам, а всі інші аргументи підуть в rest-параметри (тобто в рест підуть лише ті, які не потраплять в інші оголошені параметри)


// function f(a, b, ...args) {
//         console.log(a);   // сюди потрапить 1
//         console.log(b);   // сюди потрапить 2
//         console.log(args); // в rest параметр потраплять  3, 4, 5
// }
...
// Метод масивів .reduce() - застосовує фунцкію reducer до кожного елемента масива (з ліва на право), повертаючи одне результуюче значення
...
// arguments - є вбудованою властивістю фунцкії. 
//                 Не треба оголошувати оскільки це вбудована властивість, і використовується в тілі функції. 
//                 Має свій тип даних Arguments є псевдомасивом, має лише деякі властивості масиву, тому до нього НЕ можна використовувати методи масивів. Можна лише пошук за індексом, length, та цикли for i for...of 
//                 Приймає всі передані у функцію аргументи в т.ч. ті, що мають свої оголошені параметри.

// rest-parameters - не є властивістю функції, це просто операція. 
//                 Обов'язково потрібно оголлошувати в параметрах функції. 
//                 Має тип даних Array, тому до нього можна використовувати методи масивів. 
//                 Приймає лише залишок аргументів які не мали спеціальних оголошених параметрів у фунцкії.




// Задачка переписати задачу нижче з arguments на rest-parameters
...
// operation - arguments[0], тому цикл перебираємо з 1
                ...
                // operation = 1 аргумент '*', operandN = всі інші аргументи
...
//                         return accum + item;
//                 }
//                 accum = operands.reduce(sum);
//         } else if (operation === '*') {
//                 accum = 1;
//                 function multiple(accum, item) {
//                         return accum * item;
//                 }
//                 accum = operands.reduce(multiple);
//         } else {
//                 return undefined;
//         }
...
// Важливо! При використанні arguments чи ...rest зміщуються індекси елементів
// якщо це arguments то індекси виглядають отак (тобто 0 індекс іде з початку всіх аргументів):
                                // 0   1  2  3
//якщо це ...rest то індекси виглядають отак (тото 0 індекс посинається там де і рест парам, у нас це з другого аргумента ):
                                   //  0  1  2

// const sumCalculate = calculate('*', 2, 4, 6);


// Другий варіант рішення не використовуючи функції і метод .reduce :
...
// ########################################################################################################################################################################
                ...
                // Методи та властивості рядків (String Methods and propeties)
                
// https://uk.javascript.info/string  -- почитати, тут гарно прописано про властивості та методи рядків


// Рядки в JS це псевдомасиви. Тобто вони мають деякі властивості та методи масивів. А також ітеруються за допомогою циклів.
// Рядки в JS мають властивість length : string.length
// length це властивість а не метод тому писати отак : string.length() - неправильно, правильно string.length
// Властивість length  містить в собі довжину рядка, тобто console.log('Hello'.length) виведе число 5

// Будь який примітивний тип даних можна помістити в обгортку, чим перетворити його на обєкт і первеірити його властивості та методи.
// Наприклад number чи string можна загорнути в обгортку функції контсруктора через слово new і отримати обєкт

// new String ('Hello');
// 0: "H"1: "e"2: "l"3: "l"4: "o" length: 5 [[Prototype]]: String [[PrimitiveValue]]: "Hello"
...
// let guestList = "Гості:\n * Іван\n * Петро\n * Марія";
// console.log(guestList);  // виведе список гостей в декілька рядків

// Усі спеціальні символи починаються зі зворотного слеша \. Його також називають “символом екранування”.
// якщо нам потрібно показати зворотний слеш \ у рядку, нам потрібно подвоїти його: \\, якщо нам потрібен апостроф то записуєм його так \' - виведетться лише апостроф

// // отримуємо символ за індексом в рядку за допомогою [] (індекси такі ж як в масивах, індекс 0 це перший символ в рядку)
// console.log(str[0]);  // q
// console.log(str[5]);  // y
// console.log(str[str.length - 1]); // y

// // [] завжди повертатимуть undefined для відємних значень (якщо нам потрібно отримати символи з кінця рядка)
// console.log(str[-1]);  // undefined

// // також можна отримати доступ до символів за індексом за допомогою методів charAt() та at()
// console.log(str.charAt(0));  // q   - це устарівший але працюючий метод отримати символ за індексом з початку рядка, але не приймає відємних значень
// console.log(str.at(-1));  // y  - те ж саме, для отримання символу за індесом але приймає відємні значення для отримання символів з кінця рядка

// // Рядки НЕ можна змінювати!!!
// // рядки не змінюються якщо присвоювати їм інші символи! Змінити символ в рядку НЕ можливо
// // замінюємо останній символ (під індексом 5) на інший символ
// str[5] = 'q';  
// console.log(str);  // 'qwerty' - рядок всерівно залишиться незмінним, останній символ не заміниться на q

// // можна створити новий рядок замість старого, записавши йрнр в ту саму змінну
// let str1 = 'Yes';
// str1 = 'y' + str1[1] + str1[2];
// console.log(str1); // 'yes' - замінили першу буккву на маленьку а інші дві зконкатинували з того ж стрінга


// // можна для рядків використовувати метод slice() і конкатенацію або метод concat() щоб змінити рядок в іншій змінній
// const newstr1 = str.slice(0, 5) + 'q';
// console.log(newstr1);

// const newstr2 = str.slice(0, 5).concat('abc');
// console.log(newstr2);


// // можемо перебрати рядок посимвольно за допомогою циклу for...of
// for (let char of 'Hello Anna') {
//         console.log(char);   // виведе в консоль кожну букву стрінги окремо
// }

// // можна перевіряти рядки на наявність якихось символів чи підрядків (наприклад коли виконується пошук в тексті)
// // для цього використовується ряд методів endsWith(), startsWith(), includes()

// // startsWith() перевіряє чи починаєтьс рядок з вказаних в дужках символів і повертає булеве значеннян
// console.log(str.startsWith('qw')); // true
// console.log(str.startsWith('a')); // false

// // endsWith() перевіряє чи закінчується рядок на вказані в дужках символів і повертає булеве значеннян
// console.log(str.endsWith('y')); // true
// console.log(str.endsWith('qw')); // false

// // includes() перевіряє чи рядок вміщає в себе вказані в дужках символи (в будь якому місці) і повертає булеве значеннян
// console.log(str.includes('adc')); // false
// console.log(str.includes('wer')); // true

// // можна змінювати регістр на всі великі літери за допомогою str.toUpperCase() чи всі маленькі за допомогою str.toLowerCase()
...
// str.slice(start [, end]) - повертає частину рядка починаючи від старт не включно до енд. Другий параметр енд не обов'язковий.
// Якщо не вказати другий параметр то метод slice поверне символи від старт до кінця рядка.
// Також для старт і енд можна задати від'ємне значення, тоді позиція буде рахуватись з кінця рядка

// console.log(str.slice(0, 3)); // qwe  - індекс 3 не включається
// console.log(str.slice(1));  // werty  - від заданого індексу до кінця рдка якщо не вказано другий параметр
// console.log(str.slice(-5, -1));  // wert - починаємо з 5 символа зправа і закінчуємо на 1 символі зправа

// str.substring(start [, end]) - Повертає частину рядка між start та end (не включаючи end).Цей метод майже такий самий що і slice, але він дозволяє задати start більше ніж end (у цьому випадку він просто міняє значення start і end місцями).
// substring не підтримує відємні значення, вони інтерпретуються як 0

// ля substring ці два приклади однаокві, оскільки перше значення більше ніж друге він міняє їх місцями
// console.log(str.substring(2, 5)); // ert
// console.log(str.substring(5, 2)); // ert


// str.substr(start [, length]) - Повертає частину рядка з позиції start, із заданою довжиною length.
// На відміну від попередніх методів, цей дозволяє вказати довжину length замість кінцевої позиції:

// console.log(str.substr(2, 4));  // erty - повертає 4 символи починаючи з 2 індекса
//перший аргумент може бути від'ємний щоб рахувати з кінця
// console.log(str.substr(-4, 2)); // er - повертає 2 символи починаючи з 4 символа зправа (з кінця)



// Порівняння рядків
// рядки порівнюються символ за символом в алфавітному порядку. Але :
// 1. Літера в малому регістрі завжди більша за літеру у великому, тому при порівнянні рядків їх треба спершу привести до одного регістра:
// console.log( 'a' > 'Z' ); // true        // console.log('V' > 'o'); // false

// 2. Літери з діакритичними знаками (напр Ö та ñ) не читаються в алфавітному порядку:
// console.log('Österreich' > 'Zealand'); // true - хоча Z остання літера в алфавіті
...
// Задачка
// Написати функцію яа за рядком 'Ivo Smith' зформує ряок з ініціалами 'IS'

// function getInitials(name) {
//         // знайти пробіл 

//         const spaceIndex = name.indexOf(' ');
//         console.log(spaceIndex);  // пробіл знаходиться під індексом 3

//         // return перший символ + символ наступний після пробіла
//         return (name[0] + name.slice(spaceIndex + 1, spaceIndex + 2)).toUpperCase();
// }

// const initials = getInitials('Anna tovt');
// console.log(initials);



// Задачка 
// Напишіть функцію ucFirst(str), яка повертає рядок str з першим символом у верхньому регістрі, наприклад: ucFirst("василь") == "Василь";


// function ucFirst(str) {
//         if (!str) return str;  // якщо рядок пустий то поверни пустий рядок (інакше повернеться undefined)

//         return str[0].toUpperCase() + str.slice(1);  // якщо рядок не пустий заміни перший символ на великі літери і додай всі інші літери від 1 індекса до кінця рядка
// };
// const changedName = ucFirst('мфіня');
// console.log(changedName);

// Цей шматок коду:
// if (!str) return str; -  перевіряє, чи є рядок str порожнім або "хибним" (falsy) значенням. Якщо рядок порожній або "хибний", функція негайно повертає цей рядок.

// Пояснення:

// if (!str):
// !str - це логічне "НЕ" (!) оператор, застосований до змінної str.
// В JavaScript деякі значення вважаються "хибними" (falsy), тобто вони інтерпретуються як false в логічному контексті.
// До "хибних" значень належать: '' (порожній рядок), 0 (нуль), null, undefined, NaN (не число), false

// Отже, !str поверне true, якщо str є будь-яким з цих "хибних" значень.
// return str;:
// Якщо умова !str виконується (тобто str є "хибним" значенням), функція негайно повертає значення str.
// Це означає, що якщо у функцію передається порожній рядок, нуль, null, undefined тощо, функція поверне це ж значення без будь-яких подальших обчислень.
...
// рішення 2
// function checkSpam(str) {
//         let lowerStr = str.toLowerCase();  // перевести в нижній регістр вчхідний стрінг
      
//         return lowerStr.includes('viagra') || lowerStr.includes('xxx');  // якщо ці умови справджуються повернеться тру, якщо ні, тобто стрінг не містить вкзааних слів то повернеться фолс
// }
...
// рішення 2 за одпомогою тпернарного оператора
// function truncate(str, maxlength) {
...
// console.log(extractCurrencyValue('abc123'));
  
  
  // ##############################################################################################################################


                // Область видимості. Лексичне оточення. Замикання (Scope. Lexical Environment. Closure)


// Scope / Область видимості.

// Ми часто працюємо зі змінними і повинні розуміти де ми можемо використати ту чи іншу змінну.

// Змінні, оголошені поза будь-яким блоком - це глобальні змінні, їх видно всім блокам (Scope - Script)

//Змінні, оголошені в середині блоку, тобто в {} - це локальні змінні, їх видно лише в середині блоку (Scope - Block (для різних операторів if, while, for...)/Local (для функцій))

// const globalVar = 'global';  // це глобальна змінна, яка видима на весь скрипт, вона оголошена на найвищому рівні без будь яких блоків

// if (true) {
//         const localVarIn1If = 'local in 1 if';  // це локальна змінна оголошена в блоці, видима лише для цієї чистини коду загоргнутої в {}. Скрипт за межами цього блоку не бачить цієї змінної
//         console.log(globalVar); // тут видима і глобальна
//         console.log(localVarIn1If); // тут видима і локальна
// }
// console.log(globalVar); // тут видима бо глобальна
// console.log(localVarIn1If); // тут невидима для скриптта бо знаходиться змінна в середині блоку


// Отже з середини блоку на зовні ми бачимо всі змінні, а  ззовні в середину блока ми не бачимо (виглянути з коробочки можна. зглянути в коробочку не можна)



// Lexical Environment / Лексичне оточення

// Лексичне оточення - концепція, згідно якої оточення фунцкії формується згідно з місцем її оголошення в коді (не виклику! а саме оголошення)
// Тобто наша фунцкія f2 оголошена в глобальній області видимості шукатиме значення змінної 'a' яку ми виводим в консоль на своєму рівні де вона оголошена і якщо не знайде на своєму рівні буде шукати на рівень вище аж до глобального рівня а не в середині функції f1 - не дивлячись на те, що викликаємо ми її із фунцкії f1
// Лексичне оточення по суті це ті блоки які є видимими наприклад для функції.

// const a = 'global';  // це глобальна змінна

// function f1() {
//         const a = 'local';  // це локальна змінна
//         console.log(a);  // local
//         f2();
// }
...
// Замикання - це функція, що виконується в середовищі, яке містить одну або більше зв'язаних змінних. Функція має доступ до цих змінних під час виконання
// Замикання - це комбінації функції та лексичного оточення, в якому ця фнкція була визначена.Іншими словами, замикання дає доступ до Scope зовнішньої функції з внутрішньої функції.
// в JS замикання створюються щоразу при створенні функції під час її створення
...
// const a = 'global';  // ця змінна у нас зараз відображається в Script (якщо перевірити в debugger)

// function f1 () {
//         const a = 'local';  // ця змінна зараз відображається в Closure(якщо перевірити в debugger)
//         // змінна що потрапляє в Closure не вважається локальною Local тому що вона не оголошена в змінній f2
//         // вона вважається замиканням функції оскільки знаходиться в функції f2 на один рівень вище від f2

//         // Тобто наша фунцкія f2 оголошена в середині фунцкії f1 шукатиме значення змінної 'a' яку ми виводим в консоль на своєму рівні де вона оголошена і якщо не знайде на своєму рівні буде шукати на рівень вище в функції f1, якщо і там не знайде то дійде аж до глобального рівня - незалежно від того де саме ми її викликаємо, важливо де ми оголосили функцію f2
...
// Описана фунцкія може жити не тільки в середині f1. Насправді замикання ми бачимо в рооботі тоді, коли функція поовератється і працює десь потім в іншому місці
// Напр внесемо певні зміни в код вище: 
...
//         return f2;  // замість виклику тут, будемо повертати нашу фукцію f2
// }


// const f2FromF1 = f1();  // функція - це звичайний обєкт в JS але з ним в довісок завжди йде її оточення (environment) тобто змінна а
// f2FromF1(); // викликаємо функцію f1 що в свою чергу запускає всередині f2 яка в результат-і виводить в консоль значення змінної а - local

// Замикання це комбінація функції та лексичного оточення в якому ця фунцкія була визначена
// у Closure в Scope - локальні змінні фнкції, всередині якої виконувана функція була створена
// фнукція f2 була створена в середині функції f1, значить локальні змінні функції f1 попадуть в closure



// Задачка
// Використовуючи замикання, реалізувати лічильник

// function counter () {  // у нас є фунцкія в середині якої є локальна змінна і яка повертає іншу функцію
//         let i = 0;  // це локальна змінна, що одночано є лексичним оточенням нашої внутрішньої функції і потапляє в замикання closure

//         return function () {  // функція counter повертає іншу функцію 
//                 return ++i;
//         };
// }

// // як нам цим користуватись?
// // можемо отримати результат роботи функції counter
// const counter1 = counter();  // результат 1
...
// const counter2 = counter();  // результат 1 бо тут в іншій змінній значення почне заново рахуватись (зберігається в різних кластерах пам'яті тому що отримує з функції інший обєкт і до нього приколотилося своє лексичне оточення)
...
// // тобто щоразу зберігаючи результат функції counter  в різні змінні ми будемо отримувати в змінну інший обєкт з своїм персональним лексичним оточенням, зі своїм замиканням
// // тому наші counter1 i counter2 викликаючи одну й ту ж функцію, отримують різні обєкти з своїм індивідуальним лексичним оточенням/замиканням і живуть незалежним життям

// // якщо ми спробуємо зараз в цьому місці запустити ще counter1 то він продовжить своє життя там де він закінчив в попередньому місці
...
// Рекурсія -це коли фунція викликає саму себе
// Рекурсія – це паттерн, який є корисним у ситуаціях, коли завдання може бути розділено на кілька завдань того ж роду, але простіших.

// Напр :  // 3**0 = 1, 3**1 = 3
//3 ** 4 = 3 * 3**3 = 3 * 3 * 3 ** 2 = 3 * 3 * 3 * 3 ** 1 = 3 * 3 * 3 * 3 * 3 ** 0  // три в степені 4 ми можем розбирати покроково поділивши їх на менші підзадачі

// або з факторіалами 0! = 1 :
// 4! = 1 * 2 * 3 * 4 = 24
// 4! = 4 * 3! = 4 * 3 * 2! = 4 * 3 * 2 * 1! = 4 * 3 * 2 * 1 *0!

// тобто мета прикладів зверху розібрати складну задачу розбираючи її пошарово покроково на простіші задачі до тих пір доки не дійдем до крайньої простої точки 

// рекурсія може викликати себе приблизно 9 000 разів в глибину. Оскільки в функції factorial ми не задали де має зупинитись рекурсія то вона пішла у безкіечність і скрипт блокується з помилкою  Maximum call stack size exceeded
...
// отже будь яка рекурсія повинна містити перевірку чи не досягли ми найпростішого випадку коли рекурсія повинна припинитись
// обовязково треба передбачити умову закінчення рекурсії
...
// }
// // debugger;
// const f4 = factorial(4);  // 24


// Вивести в консолт дл я bracketSequences(2) => (()); bracketSequences(4) => (((())))

// function bracketSequences(n) {
//         if (n === 0) {
//                 return;
//         }
//         console.log('(');
//         bracketSequences(n - 1)
//         console.log(')');
// }
...
// Напишіть рекурсивну функцію factorial(n), яка обчислює факторіал заданого невід'ємного цілого числа n.

// Факторіал 0 дорівнює 1.
// Факторіал n (для n > 0) дорівнює n * factorial(n - 1).
...
// Задача 2: Сума чисел до N

// Напишіть рекурсивну функцію sumToN(n), яка обчислює суму всіх цілих чисел від 1 до заданого числа n.

// Якщо n дорівнює 1, сума дорівнює 1.
// Інакше, сума дорівнює n + sumToN(n - 1).
...
// Задача 3: Виведення чисел від 1 до N

// Напишіть рекурсивну функцію printNumbers(n), яка виводить у консоль всі цілі числа від 1 до заданого числа n.

// Якщо n дорівнює 1, вивести 1.
// Інакше, спочатку рекурсивно викликати функцію для n - 1, а потім вивести n.
...
// Задача 4: Перевірка на парність

// Напишіть рекурсивну функцію isEven(n), яка перевіряє, чи є задане ціле число n парним.

// Якщо n дорівнює 0, воно парне.
// Якщо n дорівнює 1, воно непарне.
// Якщо n більше 1, перевірте парність n - 2.
// Якщо n менше 0, перевірте парність -n.
...
// Розбір задачі 4 :
// Звичайно, давайте детально розглянемо, як працює рекурсія у вашій функції isEven:

// JavaScript

// function isEven(n) {
//     if (n === 0) {
//         return true; // Базовий випадок: 0 - парне
//     } else if (n === 1) {
//         return false; // Базовий випадок: 1 - непарне
//     } else if (n < 0) {
//         return isEven(-n); // Рекурсивний виклик для від'ємних чисел
//     } else { // n > 1
//         return isEven(n - 2); // Рекурсивний виклик для додатних чисел
//     }
// }
...
// Розбір виклику isEven(4):

// isEven(4) викликається.

// n дорівнює 4.
// Жодна з перших трьох умов (n === 0, n === 1, n < 0) не є істинною.
// Виконується блок else: return isEven(4 - 2); тобто return isEven(2);.
// Функція isEven викликає сама себе з n = 2.
// isEven(2) викликається.

// n дорівнює 2.
// Жодна з перших трьох умов не є істинною.
// Виконується блок else: return isEven(2 - 2); тобто return isEven(0);.
// Функція isEven знову викликає сама себе з n = 0.
// isEven(0) викликається.

// n дорівнює 0.
// Перша умова if (n === 0) є істинною.
// Виконується return true;.
// Базовий випадок досягнуто, і функція повертає true.
// Значення true повертається до попереднього виклику isEven(2). Таким чином, isEven(2) також повертає true.

// Значення true повертається до першого виклику isEven(4). Таким чином, isEven(4) повертає true.

// Розбір виклику isEven(7):

// isEven(7) викликається. → return isEven(5);
// isEven(5) викликається. → return isEven(3);
// isEven(3) викликається. → return isEven(1);
// isEven(1) викликається.
// n дорівнює 1.
// Друга умова else if (n === 1) є істинною.
// Виконується return false;.
// Базовий випадок досягнуто, і функція повертає false.
// Значення false повертається по ланцюжку всіх попередніх викликів. Отже, isEven(7) повертає false.

// Розбір виклику isEven(-2):

// isEven(-2) викликається.

// n дорівнює -2.
// Умова else if (n < 0) є істинною.
// Виконується return isEven(-(-2)); тобто return isEven(2);.
// Функція isEven викликає сама себе з n = 2.
// Далі процес ідентичний розбору isEven(4), який завершується поверненням true.

// Ключові моменти рекурсії в цій задачі:

// Базові випадки: Функція має чіткі умови зупинки (n === 0 і n === 1), які визначають, коли рекурсія має припинитися і повернути конкретне значення. Без базових випадків функція б викликала себе нескінченно, що призвело б до помилки переповнення стека викликів.
// Рекурсивний виклик: У випадках, коли n не є базовим, функція викликає сама себе з модифікованим аргументом (n - 2 для додатних чисел, -n для від'ємних). Цей модифікований аргумент наближає нас до одного з базових випадків.
// Повернення значення: Результат кожного рекурсивного виклику повертається до попереднього виклику, доки початковий виклик не отримає остаточний результат (true або false).
// Таким чином, рекурсія дозволяє розв'язати проблему шляхом зведення її до простіших підзадач (перевірки парності менших чисел) до тих пір, поки не буде досягнуто відомого базового випадку.



// Задача 5: Знаходження суми елементів масиву

// Напишіть рекурсивну функцію sumArray(arr), яка обчислює суму всіх чисел у заданому масиві arr.

// Якщо масив порожній, сума дорівнює 0.
// Інакше, сума дорівнює першому елементу масиву плюс сума решти елементів масиву.
...
// Базовий випадок: if (arr.length === 0) { return 0; } - як і у вас, правильно визначає суму порожнього масиву.

// Рекурсивний виклик: else { return arr[0] + sumArr(arr.slice(1)); }

// arr[0]: Отримуємо перший елемент масиву.
// arr.slice(1): Створює новий масив, що містить усі елементи arr, починаючи з індексу 1 (тобто, усі елементи, крім першого).
// sumArr(arr.slice(1)): Рекурсивно викликаємо функцію sumArr з цим новим, меншим масивом.
// arr[0] + sumArr(arr.slice(1)): Повертаємо суму першого елемента та суми решти елементів (обчисленої рекурсивно).
// Як це працює крок за кроком для sumArr([1, 2, 3]):

// sumArr([1, 2, 3]) повертає 1 + sumArr([2, 3])
// sumArr([2, 3]) повертає 2 + sumArr([3])
// sumArr([3]) повертає 3 + sumArr([])
// sumArr([]) повертає 0 (базовий випадок)
// Підставляючи значення назад:
// sumArr([3]) повертає 3 + 0 = 3
// sumArr([2, 3]) повертає 2 + 3 = 5
// sumArr([1, 2, 3]) повертає 1 + 5 = 6


// Задача 6: Піднесення до степеня

// Напишіть рекурсивну функцію power(base, exponent), яка обчислює base в степені exponent. Вважайте, що exponent є невід'ємним цілим числом.

// Якщо exponent дорівнює 0, результат дорівнює 1.
// Інакше, результат дорівнює base * power(base, exponent - 1).
...
// Задача 7: Розгортання рядка

// Напишіть рекурсивну функцію reverseString(str), яка повертає заданий рядок у зворотному порядку.

// Якщо рядок порожній, він вже розгорнутий.
// Інакше, розгорнутий рядок складається з останнього символу рядка, з'єднаного з розгорнутою рештою рядка (без останнього символу).
...
// Задача 8: Знаходження найбільшого елемента в масиві

// Напишіть рекурсивну функцію findMax(arr), яка знаходить найбільший елемент у заданому масиві чисел. Вважайте, що масив не порожній.

// Якщо масив містить лише один елемент, цей елемент є найбільшим.
// Інакше, порівняйте перший елемент масиву з найбільшим елементом решти масиву (знайденим рекурсивно) і поверніть більший з них.
...
// console.log(findMax([1, 2, 3, 4, 5, 13]));


// Як це працює крок за кроком для findMax([1, 2, 3]):

// findMax([1, 2, 3])
// restMax = findMax([2, 3])
// restMax = findMax([3])
// Базовий випадок: повертає 3
// Порівняння: 2 > 3 - false, повертає 3
// Порівняння: 1 > 3 - false, повертає 3




// Задача 9: Перевірка паліндрому (простий варіант)

// Напишіть рекурсивну функцію isPalindrome(str), яка перевіряє, чи є заданий рядок паліндромом (читається однаково з обох сторін), ігноруючи пробіли та регістр. Для простоти цього варіанту вважайте, що рядок не містить пробілів та розділових знаків.

// Якщо рядок порожній або містить один символ, це паліндром.
// Інакше, якщо перший і останній символи рядка збігаються, перевірте, чи є паліндромом решта рядка (без першого та останнього символів).
...
//                 return true;
//         } else {
//                 const typedStr = str.toLowerCase().replaceAll(' ', '');
//                 // debugger;
//                 return typedStr[0] === typedStr[typedStr.length - 1] ? isPalindrome(typedStr.slice(1, -1)) : false;
//         }
// }
...
// Задача 10: Числа Фібоначчі

// Напишіть рекурсивну функцію fibonacci(n), яка повертає n-те число Фібоначчі. 
// Послідовність Фібоначчі починається з 0 та 1, а кожне наступне число є сумою двох попередніх: 0, 1, 1, 2, 3, 5, 8, ...
// суть задачі в тому щоб вивести те число яке в ряду Фібоначчі умовно знаходиться під індексом того числа яке ми передаємо як аргумент
// Якщо n дорівнює 0, результат дорівнює 0.
// Якщо n дорівнює 1, результат дорівнює 1.
// Інакше, результат дорівнює fibonacci(n - 1) + fibonacci(n - 2).
...
// Щоб виконати тут саму задачу але з в результаті отримати ще масив всього ряду Фібоначчі до заданого нами числа є інше рішення
...
//     console.log(fibonacciSequenceIterative(5));  // (6) [0, 1, 1, 2, 3, 5]
//     console.log(fibonacciSequenceIterative(6));  // (7) [0, 1, 1, 2, 3, 5, 8]
//     console.log(fibonacciSequenceIterative(7));  // (8) [0, 1, 1, 2, 3, 5, 8, 13]



// Задача 11: Лінійна рекурсія - Сума цифр числа

// Напишіть рекурсивну функцію sumDigits(n), яка приймає невід'ємне ціле число n і повертає суму його цифр.

// Якщо n менше 10 (одна цифра), поверніть n.
// Інакше, поверніть останню цифру n плюс рекурсивний виклик sumDigits для числа n без останньої цифри.
...
// Задача 12: Хвостова рекурсія - Знаходження степеня (оптимізована)

// Перепишіть функцію power(base, exponent) з Задачі 6, використовуючи хвостову рекурсію.
// Хвостова рекурсія - це форма рекурсії, де рекурсивний виклик є останньою операцією у функції. 
// Це дозволяє деяким інтерпретаторам оптимізувати виклик, уникаючи переповнення стека для великих значень exponent. 
// (Для JavaScript оптимізація хвостової рекурсії не є гарантованою, але розуміння цього концепту корисне).

// Вам знадобиться допоміжна функція з додатковим параметром-акумулятором.


// function power(base, exponent) {
//         function helper(base, exponent, accum) {
//                 if  (exponent === 0) {
//                         return accum;
//                 } else {
//                                         //2    // 3          // 1 * 2
...
//                                         // 2  // 1          // 3 * 2
//                                         // 2  // 0          // 4 * 2
//                         return helper(base, exponent - 1, accum * base);
//                 }
//         }
...
// helper(2, 3, 1)
// != база рекурсії
// helper(2, 2, 1 * 2) => helper(2, 2, 2)
// Роман Freshcode
// 14:50
// ітерація 3:
// helper(2, 2, 2)
// != база рекурсії
// helper(2, 1, 2 * 2) => helper(2, 1, 4)
// ітерація 4:
// helper(2, 1, 4)
// != база рекурсії
// helper(2, 0, 4 * 2) => helper(2, 0, 8)
// Роман Freshcode
// 14:52
// ітерація 5:
// helper(2, 0, 8)
...
// Задача 13: Деревоподібна рекурсія - Обхід простого дерева (вузли зі значенням)

// Уявіть собі просте дерево, де кожен вузол має value та може мати масив children (інших вузлів). Напишіть рекурсивну функцію traverseTree(node), яка приймає кореневий вузол дерева та виводить у консоль значення всіх вузлів дерева.
...
// Задача 14: Взаємна рекурсія - Перевірка парності (інший підхід)

// Напишіть дві функції: isEvenMutual(n) та isOddMutual(n), які взаємно викликають одна одну, щоб визначити, чи є задане невід'ємне ціле число n парним або непарним.

// isEvenMutual(n) повертає true, якщо n парне, і викликає isOddMutual(n - 1) інакше (для n > 0).
// isOddMutual(n) повертає true, якщо n непарне, і викликає isEvenMutual(n - 1) інакше (для n > 0).
// Базові випадки: isEvenMutual(0) повертає true, isOddMutual(0) повертає false.
// Задача 15: Рекурсія зі списком - Знаходження кількості елементів, що задовольняють умову

// Напишіть рекурсивну функцію countIf(list, condition), яка приймає зв'язаний список (представлений об'єктами з властивостями value та rest - вказівником на наступний елемент, або null для кінця списку) та функцію condition. Функція повинна повернути кількість елементів у списку, для яких функція condition(value) повертає true.
